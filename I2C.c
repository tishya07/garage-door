...
// done? small changes but mostly copypasted from lab 4
...
void I2C_GPIO_Init(void) {
	//TODO -> done?? copy pasted kinda from lab 4
	// NOTE: SCL = used to PB6, but switched to PB8
	// SDA = used to be PB7, but switched to PB9
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOBEN; //8.4.17 enable gpio b clock
	
	//config PB8 as I2C1_SCL
	GPIOB->MODER &=~GPIO_MODER_MODE8; //clear 8 -> CHECK
	GPIOB->MODER |= GPIO_MODER_MODE8_1; //set as AF mode (10)
	
	GPIOB->AFR[1] &=~GPIO_AFRH_AFRH0; //clearing -> CHECK
	GPIOB->AFR[1] |= GPIO_AFRH_AFSEL8_2; //I2C1_SCL is AF4 0100 -> CHECK
	
	//config PB9 as I2C1_SDA
	GPIOB->MODER &=~GPIO_MODER_MODE9; //clear 9 -> CHECK
	GPIOB->MODER |= GPIO_MODER_MODE9_1; //set as AF mode (10)
	
	GPIOB->AFR[1] &=~GPIO_AFRH_AFRH1; //clearing -> CHECK
	GPIOB->AFR[1] |= GPIO_AFRH_AFSEL9_2; //I2C1_SDA is AF4 0100 -> CHECK
	
	//both operate at very high speed
	GPIOB->OSPEEDR |= GPIO_OSPEEDR_OSPEED8; // (11) = high speed set for PB6
	GPIOB->OSPEEDR |= GPIO_OSPEEDR_OSPEED9; // (11) = high speed set for PB7
	
	//both set to open-drain
	GPIOB->OTYPER |= GPIO_OTYPER_OT8; // set 1 open-drain for PB6
	GPIOB->OTYPER |= GPIO_OTYPER_OT9; // set 1 open-drain for PB7
	
	//both to pull-up
	GPIOB->PUPDR &=~ GPIO_PUPDR_PUPD8; // clearing
	GPIOB->PUPDR |= GPIO_PUPDR_PUPD8_0; // (01) is pull-up
	
	GPIOB->PUPDR &=~ GPIO_PUPDR_PUPD9; // clearing
	GPIOB->PUPDR |= GPIO_PUPDR_PUPD9_0; // (01) is pull-up
}
...
//===============================================================================
//                           I2C Read Temperature
//=============================================================================== 
uint8_t Temp_Read(void) {
    uint8_t SecondaryAddress = 0b1001000 << 1;  // AO = 0
    uint8_t Data_Send = 0;
    ...
    return Data_Receive;  // raw temp in °C
}
